...
// It also checks the summary counts the README and docs quote against the live count; --fix rewrites them.
...
const DOC_COUNTS: { file: string; re: RegExp; want: number[] }[] = [
  ...
  { file: 'README.md', re: /\*\*(\d+) citations over (\d+) sources\*\*/d, want: [cited, bib.size] },
  { file: 'README.md', re: /All (\d+) entries' clinical prose is translated/d, want: [entries] },
  { file: 'README.md', re: /\| Atıflar \| (\d+) \| (\d+) kaydın tamamında, (\d+) açık erişimli kaynağa \|/d, want: [cited, entries, bib.size] },
  { file: 'README.md', re: /\*\*(\d+) kaynak üzerinden (\d+) atıf\*\*/d, want: [bib.size, cited] },
